 ...
 */
export function AddToCart({ variants, label = 'Add to Cart', block, idPrefix, onVariantChange }: {
  variants: ShopVariant[]; label?: string; block?: boolean; idPrefix: string; onVariantChange?: (v: ShopVariant) => void;
}) {
  ...
  return (
    <div className={`addtocart${block ? ' addtocart--block' : ''}`}>
      {variants.length > 1 && (
        <select id={`${idPrefix}-variant`} className="addtocart__select" aria-label="Option" value={variantId} onChange={e => { setVariantId(e.target.value); const v = variants.find(x => x.id === e.target.value); if (v) onVariantChange?.(v); }}>
